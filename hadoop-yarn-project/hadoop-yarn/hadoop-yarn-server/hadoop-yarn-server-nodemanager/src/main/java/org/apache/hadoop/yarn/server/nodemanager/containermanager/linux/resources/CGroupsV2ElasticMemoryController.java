 ...
 *
 * A memory.high event is only a wake-up: whether it justifies a kill is the
 * verdict of {@link CGroupsV2OOMHandler}, which waits for it. And since
 * memory.max is a kill the kernel performs by itself, an out of memory
 * condition the handler cannot resolve is not a reason to stop: there is no
 * watchdog, no timeout and no stopListening() on this version.
 */
class CGroupsV2ElasticMemoryController extends CGroupElasticMemoryController {
  ...
  /**
   * Run the handler and keep listening whatever it did. The kernel enforces
   * the limit, so there is nothing to time out on: an out of memory condition
   * the handler does not resolve, the kernel resolves at memory.max. Stopping
   * here would leave the node with no elastic memory control at all.
   */
  @Override
