...
| E7 | Path traversal or injection through `<user>` / `<app_id>` | Both are regex-validated before use (`^application_[0-9]+_[0-9]+$`; a user name that carries no `/`, whitespace or control character, does not start with `-`, is neither `.` nor `..`, resolves through `getent`, has uid ≥ `MIN_UID` and is not in `BANNED_USERS`. The name check is deliberately about the characters that matter for a path component rather than about the shape of the name: `firstname.lastname` is a normal LDAP account, and `container-executor` pattern-matches nothing at all, it just calls `getpwnam`. The uid and banned-list checks are the same **shape** as `check_user` in `container-executor`, but **not** the same source: the script carries its own `MIN_UID` and `BANNED_USERS` rather than reading `min.user.id`, `banned.users` and `allowed.system.users` from `container-executor.cfg`, so a cluster that tunes any of those must mirror the change in `yarn-reown-orphan-app-dir.conf`, where both are required rather than defaulted). No path comes from argv, and `sudo` passes argv rather than a shell string |
...
Re-owning the local directories of <app> failed: <script> rejected user '<user>' or the
  application id                                                                 (exit 64)
    -> the user is banned, below MIN_UID, or does not resolve on this node. The name itself
       is only checked for what makes a path component unsafe — no '/', no whitespace or
       control character, no leading '-', not '.' or '..' — so an ordinary LDAP name like
       firstname.lastname is fine.
