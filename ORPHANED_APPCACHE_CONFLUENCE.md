...
| A directory being created right now is never selected | minimum-age guard on the directory mtime |
...
* `min-age-ms` is compared against the **modification time of the application directory**, taking
  the most recent value observed across all local dirs. It exists to make a race with a directory
  being created concurrently with the scan impossible in practice; there is no reason to lower it
  below an hour.
...
The helper only ever runs against an application the NodeManager no longer tracks whose directories
have been idle for `min-age-ms` (default 6 h), so **no live container process is racing it**.
...
| E8 | Chowning a live application's directories | The reaper only selects applications absent from `getApplications()` and idle for `min-age-ms`; the chown happens before the synthetic application is registered, and `putIfAbsent` still guards the race |
